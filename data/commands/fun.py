...
class FunCommands(commands.Cog):
    ...
    async def meme(self, ctx):
        ...
        # setting embed
        img = str(meme.url)
        embed=discord.Embed(title=meme.title, url=img, color=discord.Colour.from_rgb(randint(0, 255), randint(0, 255), randint(0, 255)))
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            embed=discord.Embed(title="Missing Arguments!", description="Please also tell me how many memes to send! additionally you can write \"dank\" after the amount to get the memes from dank subreddit\nFor e.g. `-sendMemes 10 dank`", color=0xff0000)
            await ctx.send(embed=embed)
