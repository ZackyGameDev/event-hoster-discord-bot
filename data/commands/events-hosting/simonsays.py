...
class SimonSays(commands.Cog):
    ...
    async def on_ready(self):
        console_log("Loading all emojis from the JSON", "yellow")
        self.emojis = json.loads(read_file('id-list.json'))['emojis']
        console_log("emojis loaded from JSON", "green")
    ...
    async def simon_says(self, ctx, *, to_say):
        if ctx.guild.get_role(self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)]['roles']['simon_says_controller']) not in ctx.author.roles:
            await ctx.send(embed=discord.Embed(
                ...
                description=f"You don\'t have the <@&{self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)]['roles']['simon_says_controller']}> role to perform this command! {self.emojis['crossGif']}",
                color=discord.Color.red()
            ))    
            return
        
        simon_says_channel = self.client.get_channel(self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)]['channels']['simon_says_channel'])
    ...
    async def simon_left_alive(self, ctx):
        ...
        for mem in ctx.guild.members:
            if ctx.guild.get_role(self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)]['roles']['simon_says_participant']) in mem.roles:
    ...
    async def simon_kill(self, ctx, *to_kill : discord.Member):
        ...
        if ctx.guild.get_role(simon_controller_role_id) in ctx.author.roles:
            for member in to_kill:
                if ctx.guild.get_role(simon_disqualified_role_id) in member.roles:
                    killed = "\n" + killed + member.mention + " was already dead"
                else:
    ...
    async def simon_revive(self, ctx, *to_revive : discord.Member):
        ...
        if ctx.guild.get_role(simon_controller_role_id) in ctx.author.roles:
            for member in to_revive:
                if ctx.guild.get_role(simon_participant_role_id) in member.roles:
    ...
    async def simon_clear_up(self, ctx):
        ...
        if ctx.guild.get_role(simon_controller_role_id) in ctx.author.roles:
            ...
            await ctx.send(embed=discord.Embed(title="Only the registerred simon says controllers can use this command!", color=discord.Color.red()))
    ...
    async def simon_says_setup(self, ctx):
        ...
        try:
            self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)].update(id_list_to_save)
        except KeyError:
            self.client.id_list['guild_setup_id_saves'][str(ctx.guild.id)] = id_list_to_save
