...
class SimonSays(commands.Cog):
    ...
    async def simon_says_start(self, ctx: commands.Context, *, stop_after: str):
        ...
        simon_participant_role: discord.Role = ctx.guild.get_role(simon_participant_role_id)
        simon_controller_role: discord.Role = ctx.guild.get_role(simon_controller_role_id)
        ...
        try:
            ...
            asyncio.create_task(edit_participation_embed(self, participation_message, stop_after=stop_after))
        except: 
        ...
        async def remove_participants_loop(timeout=stop_after*60):
            while True:
                try:
                    ...
                    user: discord.Member = self.client.get_guild(payload.guild_id).get_member(payload.user_id)
                    try:
    ...
    async def clear_error(self, ctx, error):
        if isinstance(error, commands.BotMissingPermissions):
            ...
            console_log(f"error in SimonSays.simon_says_start! {error}", "white", "on_red")
